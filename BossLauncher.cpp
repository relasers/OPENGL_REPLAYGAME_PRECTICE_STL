...
bool CBossLauncher::Update(const float fTimeElapsed)
{
	...
	if (m_shoot_tick > m_cooltime*0.1) {
		...
		target.x += -128 + random_target(dre);
		target.y += -128 + random_target(dre);
		target.z += -128 + random_target(dre);
	...
	if (m_energybolt_tick > m_cooltime) {
		...
		for (int i = 0; i < 4; ++i) {
			Vec3f target = m_Target->GetCenter();
			target.x += -128 + random_target(dre) + cos(i * 90 * PI / 180.0) * 1280;
			target.y += -128 + random_target(dre);
			target.z += -128 + random_target(dre) + sin(i * 90 * PI / 180.0) * 1280;
