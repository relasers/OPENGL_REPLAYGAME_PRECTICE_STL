...
void CMainScene::SettingObjects()
{
	...
	m_pCamera->RotateViewDirection(0, 0);
	m_pCamera->TranslatePos(0.0f, 0.0f, 0.0f);
	...
	uniform_int_distribution<> random_pos(0, MapSize);
	uniform_int_distribution<> random_xz_scale(0, 150);
	uniform_int_distribution<> random_y_scale(0, 1200);
	uniform_int_distribution<> random_pitch_roll(0, 10);
	uniform_int_distribution<> random_yaw(0, 90);
...
void CMainScene::Update(float fTimeElapsed)
{
	...
	if (m_Player->IsDie())
	{
		...
		else if (!m_bGameOver)
		{
			...
			if (m_CommandManager->GetCommandMode() == CCommandManager::CommandMode::GamePlay)
			{
				cout << "//////////////////////////" << endl;
				cout << "Game Over!!!!" << endl;
				cout << "//////////////////////////" << endl;
...
bool CMainScene::OnProcessingMouseMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	switch (m_CommandManager->GetCommandMode())
	{
	case CCommandManager::CommandMode::GamePlay:
	{
		switch (nMessageID)
		{
		case WM_LBUTTONDOWN:
			...
			m_isShoot = true;
			m_GameUpdateElapsed = chrono::system_clock::now() - m_pFramework->GetGameTime();
			m_CommandManager->InsertCommand({ CCommand::CommandType::MouseShoot,0,0,0,0,0,true,  m_GameUpdateElapsed.count() });
			break;
		case WM_LBUTTONUP:
			m_Player->SetisShooting(false);
			m_isShoot = false;
			m_GameUpdateElapsed = chrono::system_clock::now() - m_pFramework->GetGameTime();
			m_CommandManager->InsertCommand({ CCommand::CommandType::MouseShoot,0,0,0,0,0,false, m_GameUpdateElapsed.count() });
			break;
		case WM_RBUTTONDOWN:
			break;
		case WM_RBUTTONUP:
		...
		case WM_MOUSEMOVE:
			...
			m_GameUpdateElapsed = chrono::system_clock::now() - m_pFramework->GetGameTime();
			m_CommandManager->InsertCommand({ CCommand::CommandType::MouseMove,lParam,m_isShoot,0,0,0,0, m_GameUpdateElapsed.count() });
			break;
		}
	...
	case CCommandManager::CommandMode::GameReplay:
	{
...
void CMainScene::BuildObjects(CFramework * pFramework, HWND hwnd, CurrentScene tag)
{
	CScene::BuildObjects(pFramework, hwnd, tag);
	m_SoundManager = m_pFramework->GetSoundManager();
	m_CommandManager = m_pFramework->GetCommandManager();
...
bool CMainScene::ProcessInput()
{
	...
	case CCommandManager::CommandMode::GamePlay:
	{
		...
		if (!bProcessedByScene)
		{
			...
			m_GameUpdateElapsed = chrono::system_clock::now() - m_pFramework->GetGameTime();
			m_CommandManager->InsertCommand({ CCommand::CommandType::KeyBoardType,0,m_bFront,m_bBack,m_bLeft,m_bRight,bPushShift, m_GameUpdateElapsed.count() });
	...
	case CCommandManager::CommandMode::GameReplay:
	{
		...
		if (GetKeyboardState(pKeyBuffer))
		{
			if (pKeyBuffer['1'] & 0xF0) { m_pCamera->SetViewMode(CCamera_OpenGL::CameraViewMode::TPS); }
			if (pKeyBuffer['2'] & 0xF0) { m_pCamera->SetViewMode(CCamera_OpenGL::CameraViewMode::FPS); }
			else if (pKeyBuffer['Q'] & 0xF0) { PostQuitMessage(0); }
		}
		...
		while (m_CommandManager->CheckExecutablaKeyboardCommand(m_GameUpdateElapsed.count())) {
			CCommand* command = m_CommandManager->ExecuteKeyCommand();

			m_bFront = command->GetCommand().bFront;
			m_bBack = command->GetCommand().bBack;
			m_bLeft = command->GetCommand().bLeft;
			m_bRight = command->GetCommand().bRight;
			m_isboost = command->GetCommand().bswitched;
		}

		while (m_CommandManager->CheckExecutableMouseMoveCommand(m_GameUpdateElapsed.count())) {
			CCommand* command = m_CommandManager->ExecuteMouseMoveCommand();
			m_pCamera->RotateViewDirection(LOWORD(command->GetCommand().lParam), HIWORD(command->GetCommand().lParam));
		}

		while (m_CommandManager->CheckExecutableMouseShootCommand(m_GameUpdateElapsed.count())) {
			CCommand* command = m_CommandManager->ExecuteMouseShootCommand();
			m_Player->SetisShooting(command->GetCommand().bswitched);
		}

		m_Player->Boost(m_isboost);
