...
bool CTitleScene::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case VK_F1:
			...
			m_pFramework->SetCommandMode(CCommandManager::CommandMode::GameReplay);
			m_pFramework->ChangeScene(CScene::CurrentScene::Main, true);

			

			m_pFramework->ResetGameTimeElapsed();
		...
		case VK_F2:
			...
			m_pFramework->SetCommandMode(CCommandManager::CommandMode::GamePlay);
			m_pFramework->ChangeScene(CScene::CurrentScene::Main, true);

			

			m_pFramework->ResetGameTimeElapsed();
			m_pFramework->InitializeCommandManager();
			m_timer = 0.0f;
			m_Input = false;
			break;
		}
