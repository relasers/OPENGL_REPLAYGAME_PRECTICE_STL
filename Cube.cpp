...
CCube::CCube(CTextureLibraray * texture,
	float random_xz_scale,
	float random_y_scale,
	float random_pitch_roll,
	float random_yaw,
	float random_pos
)
{
	...
	m_Center.x = static_cast<float>(random_pos - MapSize / 2);
	m_Center.y = 0.0f;
	m_Center.z = static_cast<float>(random_pos - MapSize / 2);
	...
	m_scale.x = 50.0f + random_xz_scale;
	m_scale.y = 500.0f + random_y_scale;
	m_scale.z = 50.0f + random_xz_scale;
	for (int i = 0; i < 16; i++) m_matrix[i] = 0;
	for (int i = 0; i < 4; i++) m_matrix[i * 4 + i] = 1;
	m_Angle.pitch = 5 - random_pitch_roll;
	m_Angle.yaw = 45 - random_yaw;
	m_Angle.roll =  5 - random_pitch_roll;
